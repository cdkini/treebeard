...
def _write_config(cfg_dir: Path, vault: Path) -> None:
    cfg_dir.mkdir(parents=True, exist_ok=True)
    (cfg_dir / "config.toml").write_text(f'vault = "{vault}"\n', encoding="utf-8")


def test_appends_line_when_vault_exists(tmp_path: Path) -> None:
...
def test_silently_skips_when_no_config(tmp_path: Path) -> None:
...
def test_silently_skips_when_vault_missing(tmp_path: Path) -> None:
    cfg_dir = tmp_path / "cfg"
...
def test_silently_skips_when_om_dir_missing(tmp_path: Path) -> None:
