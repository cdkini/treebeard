"""Move notes into `<vault>/.om/archive/` with a timestamped prefix.

Used by `om archive` for user-selected notes and by the auto-indexer for
...
def archive_paths(
    ...
    """Move each `paths` entry into `<vault>/.om/archive/` with a shared
    `archive_stamp(now)` prefix. Returns the new locations in the same
    order. Missing sources raise `OmError`."""
    if not paths:
        return []
