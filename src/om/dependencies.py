"""Registry of external binaries om shells out to.
...
# Hard requirements: every `om` invocation checks these at CLI startup,
...
def first_available(deps: tuple[Dependency, ...]) -> Dependency | None:
    ...
    Used by `om init` to pick a sensible default for the user."""
    return next((dep for dep in deps if dep.is_available()), None)


def check_all() -> None:
    """Raise `OmError` for the first missing required dependency.
    ...
    for dep in REQUIRED:
        if not dep.is_available():
            raise OmError(f"{dep.label} is required", hint=dep.install_hint)
...
def check_editor(name: str) -> None:
    ...
    if shutil.which(name) is None:
        raise OmError(
            ...
            hint="install it, or change `editor` in your om config",
        )
