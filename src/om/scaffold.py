...
def compose_claude_md() -> str:
    ...
    Loaded from `om/prompts/CLAUDE.md` via `importlib.resources` so
    ...
    under `src/om/` by default). Static — same bytes every time."""
    return resources.files("om").joinpath("prompts/CLAUDE.md").read_text(encoding="utf-8")
