...
from om.frontmatter import TIMESTAMP_FMT
...
class GranolaImporter:
    ...
    def _get(self, path: str, *, params: dict[str, Any]) -> dict[str, Any]:
        try:
            ...
            raise OmError(f"granola API request failed: {exc}") from exc
        ...
        if response.status_code >= 400:
            raise OmError(f"granola API error {response.status_code}: {response.text[:200]}")
