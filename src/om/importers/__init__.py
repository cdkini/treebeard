"""Importer interface shared by all `om import <foo>` integrations.

An `Importer` knows how to fetch notes from one external system. The
`sync()` driver in `om.importers.sync` consumes any `Importer` and lands
...
To add a new integration:
  ...
  2. Register a Click subcommand on `command` in `om.commands.import_`
