...
The `_on_close` hook in `om.cli` auto-commits whatever this writes; we
...
def sync(
    ...
    *,
    ...
    are logged and skipped — same posture as `om index` for hand-written
    files.
    """
...
def _apply(
    ...
    del now  # Kept in the signature for symmetry with om.index — we
